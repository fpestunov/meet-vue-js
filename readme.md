...
## IV. Вычисляемые свойства и слежение
...
Вычисляемые свойства кэшируются, а методы нет. Обновляется только при изменении зависимости.
...
Пример с функцией `Now`. Выдает разное время.

### Вычисляемые свойства и слежение

За слежение отвечает `watch`.
https://ru.vuejs.org/v2/api/#watch

Но вычисляемые свойства также, отслеживают все автоматически.
...
Здесь пример получения данных от АПИ:
https://ru.vuejs.org/v2/guide/computed.html#Методы-наблюдатели
